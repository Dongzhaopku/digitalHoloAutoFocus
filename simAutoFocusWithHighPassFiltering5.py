...
from subRoutine4Holo import least_squares
...
#                              )
#     return result.x, result.fun
def find_optimal_z(input_spectra, FX, FY, wavelength, region=None, tolerance=1e-3):

    def optimize_in_subinterval(bounds):
        """
        Optimize in a specific subinterval by performing the optimization once.
        """
        result = minimize_scalar(Md_reverse_fresnel_diffraction,
                                 bounds=bounds,
                                 args=(input_spectra, FX, FY, wavelength, region),
    ...
    # # First, perform optimization on the entire interval
    # initial_z, initial_min_Md = optimize_in_subinterval(bounds)

    # Split the interval into two subintervals and optimize separately
    mid_point = (bounds[0] + bounds[1]) / 2
    left_bounds = [bounds[0], mid_point]
    right_bounds = [mid_point, bounds[1]]

    # Optimize in the left and right subintervals
    left_z, left_min_Md = optimize_in_subinterval(left_bounds)
    right_z, right_min_Md = optimize_in_subinterval(right_bounds)

    # Return the z value corresponding to the minimum Md value
    if left_min_Md < right_min_Md:
        return left_z, left_min_Md
    else:
        return right_z, right_min_Md
...
def compress_complex_amplitude(complex_amplitude, a, pixel_size, region=None):
    ...
    regions -- 可选参数，输入的多个区域（每个区域为 [x1, y1, x2, y2] 的列表）

    返回:
    compressed_amplitude -- 压缩后的复振幅矩阵
    pixel -- 压缩后的像素大小
    new_regions -- 压缩后的多个区域坐标列表（如果输入了 regions）
    """
    ...
    if region is not None and a != 1:
        ...
        new_regions = region  # 直接返回原区域

    return compressed_amplitude, pixel_size1, new_regions
...
if __name__ == "__main__":
    ...
    U0 = anglular_spectrumm_diffraction_filter(Uc, -350.20E-3, wavelength, pixel_size)
    plt.figure(figsize=(6, 6))
